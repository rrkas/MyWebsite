        ...
        for materials in StudyMaterials.get_1_10_materials():
            temp.append(
                MaterialSummary(
                    ...
                    books=len([material for material in materials if material.type == StudyMaterial.BOOK]),
                    notes=len([material for material in materials if material.type == StudyMaterial.NOTE]),
                    questions=len(
                        [material for material in materials if material.type == StudyMaterial.QUESTION_PAPER]),
                    subjects=list(sorted(set([material.subject for material in materials]))),
                    languages=list(sorted(set([material.language for material in materials]), ))
                )
            )
        return temp
